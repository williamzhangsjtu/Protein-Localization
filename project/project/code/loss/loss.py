...
class LogLoss(nn.Module):
    ...
    def forward(self, logit, labels,epoch=0):
        labels = labels.float()
        logit = logit.view(-1, self.__classes_num)
        labels = labels.view(-1, self.__classes_num)

        loss = self.classify_loss(logit, labels)
        return loss
...
if __name__ == "__main__":
    label = torch.tensor([[1, 0, 0],
                          [0, 1, 1]])
    label_pred = torch.tensor([[0.9, 0.1, 0.1],
                               [0, 1, 1]])
    loss = FocalSymmetricLovaszHardLogLoss()
    print(loss(label_pred, label))