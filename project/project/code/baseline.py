...
import argparse, os
import time
from dataloader import dataloader_single, dataloader_multiple, dataloader_test, oversample_dataloader
import model as M
import utils
...
def one_epoch(model, optimizer, criterion, dataloader, iftrain=True, grad_clip=None):
    ...
    for idx, (images, labels, nums, _) in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(iftrain):
            ...
            loss = criterion(outputs[:2], outputs[2], labels)
            ...
            if (iftrain):
               ...
               optimizer.step() 
    #if iftrain: dataloader.dataset.re_sample()
